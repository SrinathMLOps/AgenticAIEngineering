...
Intermediate — Logger
======================
Adds one new panel on top of the Simple logger:

  log_token_usage(input_tokens, output_tokens)
  ─────────────────────────────────────────────
  Shown at the end of every run. Displays the total tokens consumed and an
  estimated cost in USD based on Claude Sonnet 4 pricing.

  Pricing used (per million tokens, mid-2025):
    Input:  $3.00
    Output: $15.00
"""
...
_INPUT_COST_PER_M = 3.0    # USD per million input tokens
_OUTPUT_COST_PER_M = 15.0  # USD per million output tokens


def log_token_usage(input_tokens: int, output_tokens: int) -> None:
    """
    Display cumulative token counts and estimated USD cost for the run.
    The usage object is returned by every Anthropic API response — we simply
    accumulate .usage.input_tokens and .usage.output_tokens across all steps.
    """
